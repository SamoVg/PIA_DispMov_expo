  ...
  useEffect(() => {
    fetch(
      ...
      .then((data) => {
        ...
        const transformedData = data.data.map((alumno) => ({
          label: alumno.nombres,
          value: alumno.matricula.toString(), // mejor usar string para value en dropdowns
        }));
    ...
    if (photo?.uri) {
      try {
        ...
        const response = await fetch(
          `https://engaged-pup-nearby.ngrok-free.app/FaceRecognition/RegistrarCara/${nombre}?matricula=${seleccionado}`,
          {
            method: "POST",
            headers: {
  ...
  const renderPicture = () => {
    return (
      <View>
        ...
        />
        <Button onPress={() => setUri(null)} title="Registrar otro rostro" />
      </View>
    );
  };
